...
async def _find_candidate_doc(db: Any, test_id: str, user_key: str) -> Optional[Dict[str, Any]]:
    ...
    for query in queries:
        doc = await db.cloud_test_candidates.find_one(query)
        if doc:
            return doc
    return None
...
def _send_feedback_email(candidate_email: str, subject: str, html_content: str) -> None:
    ...
    try:
        ...
        logger.error("SendGrid cloud feedback email HTTP error (%s): %s", exc.code, body)
        raise HTTPException(status_code=502, detail="Failed to send feedback email") from exc
    except urlerror.URLError as exc:
        logger.error("SendGrid cloud feedback email network error: %s", exc)
        raise HTTPException(status_code=502, detail="Failed to send feedback email") from exc
...
def _build_invitation_html(
    ...
    message = str(template.get("message") or "You have been invited to take a Cloud assessment. Please click the link below to start.")
      ...
      <div class="container">
        ...
        <div class="card">
          ...
          <p style="text-align:center;margin:24px 0;">
            <a href="{exam_url}" class="btn">Start Cloud Assessment</a>
          </p>
...
async def remove_candidate(test_id: str, user_id: str, request: Request) -> Dict[str, Any]:
    ...
    await db.cloud_test_candidates.delete_one({"_id": candidate_doc["_id"]})
...
async def send_invitation(test_id: str, request: Request, body: Dict[str, Any] = Body(...)) -> Dict[str, Any]:
    ...
    candidate = await db.cloud_test_candidates.find_one({"test_id": test_id, "email": email})
    if not candidate:
        raise HTTPException(status_code=404, detail="Candidate not found")
    ...
    exam_url = (
        f"{base_url}/cloud/tests/{test_id}/entry?token={urlparse.quote(test_token)}"
        f"&email={urlparse.quote(email)}&name={urlparse.quote(candidate_name)}"
    )
    template = test_doc.get("invitationTemplate")
    template = template if isinstance(template, dict) else {}
    subject = f"Cloud Assessment Invitation - {str(test_doc.get('title') or 'Cloud Assessment')}"
    html_content = _build_invitation_html(
        test_title=str(test_doc.get("title") or "Cloud Assessment"),
    ...
    now = datetime.utcnow()
    await db.cloud_test_candidates.update_one(
        {"_id": candidate["_id"]},
...
async def send_invitations_to_all(test_id: str, request: Request) -> Dict[str, Any]:
    ...
    template = template if isinstance(template, dict) else {}
    test_title = str(test_doc.get("title") or "Cloud Assessment")
    subject = f"Cloud Assessment Invitation - {test_title}"

    now = datetime.utcnow()
    candidates = await db.cloud_test_candidates.find({"test_id": test_id}).to_list(length=5000)
    if not candidates:
        return {
            "success": True,
...
async def verify_candidate(test_id: str, request: Request, body: Dict[str, Any] = Body(...)) -> Dict[str, Any]:
    ...
    candidate_doc = await db.cloud_test_candidates.find_one({"test_id": test_id, "email": email})
    if not candidate_doc:
...
async def get_candidate_analytics(test_id: str, user_id: str, request: Request) -> Dict[str, Any]:
    ...
    if not question_analytics:
        ...
        if object_ids:
            qdocs = await db.cloud_questions.find({"_id": {"$in": object_ids}}).to_list(length=len(object_ids))
        by_id = {str(q.get("_id")): q for q in qdocs}
...
async def send_candidate_feedback(
    ...
    candidate_doc = await _find_candidate_doc(db, test_id, user_id)
    if not candidate_doc and isinstance(body, dict):
        fallback_email = _normalize_email(str(body.get("email") or ""))
        if fallback_email and "@" in fallback_email:
            candidate_doc = await db.cloud_test_candidates.find_one({"test_id": test_id, "email": fallback_email})
    if not candidate_doc:
        raise HTTPException(status_code=404, detail="Candidate not found")
    ...
    test_title = str(test_doc.get("title") or "Cloud Test")
    ...
    <div style="font-family: Arial, sans-serif; color: #0f172a; max-width: 720px; margin: 0 auto;">
      <h2 style="color: #1d4ed8; margin-bottom: 8px;">Your Cloud Assessment Feedback</h2>
