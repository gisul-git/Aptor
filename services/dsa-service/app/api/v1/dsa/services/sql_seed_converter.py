...
def validate_seed_sql(seed_sql: str) -> bool:
    """
    Basic validation of seedSql format.
    
    Args:
        seed_sql: The seedSql string to validate
        
    Returns:
        True if valid, False otherwise
    """
    if not seed_sql or not seed_sql.strip():
        return False
    
    # Check for at least one CREATE TABLE
    if "CREATE TABLE" not in seed_sql.upper():
        logger.warning("[SQL Seed Converter] seedSql does not contain CREATE TABLE statement")
        return False
    
    # Check for balanced parentheses (basic check)
    if seed_sql.count("(") != seed_sql.count(")"):
        logger.warning("[SQL Seed Converter] seedSql has unbalanced parentheses")
        return False
