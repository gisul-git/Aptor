...
# AIML module integration - HTTP client only (no fallback)
from .....utils.service_clients import get_aiml_client

_aiml_client = get_aiml_client()
AIML_AVAILABLE = True

async def aiml_generate_question(*args, **kwargs):
    """Call AIML service via HTTP to generate questions."""
    return await _aiml_client.generate_question(
        title=kwargs.get("title", ""),
        skill=kwargs.get("skill", "Python"),
        topic=kwargs.get("topic"),
        difficulty=kwargs.get("difficulty", "medium"),
        dataset_format=kwargs.get("dataset_format", "csv")
    )
