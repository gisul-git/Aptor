    ...
    if (!config.baseURL || config.baseURL === '') {
      if (!baseUrlPromise) {
        baseUrlPromise = getApiGatewayUrl().then(url => {
          cloudApi.defaults.baseURL = `${url}/api/v1/cloud`
          return url
        })
      }
      await baseUrlPromise
      config.baseURL = cloudApi.defaults.baseURL
    }
    return config
  },
    ...
    if (typeof window !== 'undefined') {
      ...
      if (isAdminEndpoint) {
        ...
        if (isPublicCandidateEndpoint) {
          console.debug('[cloudApi] Skipping auth for public candidate endpoint:', config.url)
          return config
        }
      }
      ...
      try {
        ...
        console.warn('[cloudApi] NextAuth session not available, using localStorage fallback')
      }
      ...
      if (!token) {
        try {
          ...
          console.warn('[cloudApi] localStorage not available')
        }
      }
      ...
      if (token) {
        ...
        console.debug('[cloudApi] Authorization token added to request:', config.url)
      } else {
        console.error('[cloudApi] SECURITY WARNING: No authentication token found for Cloud API request:', config.url)
      }
    }
    return config
  },
...
export const cloudCandidateApi = {
  verifyCandidate: async (
    ...
    const response = await cloudApi.post(`/tests/${testId}/verify-candidate`, {
      token,
      email,
      name,
    })
