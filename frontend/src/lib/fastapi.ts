...
fastApiClient.interceptors.request.use(
  async (config) => {
    ...
    // Check for auth routes - both /api/v1/auth/ and /api/auth/ patterns
    ...
    console.log("🟡 [FastAPI Client] Request interceptor:", {
      url: config.url,
...
fastApiClient.interceptors.response.use(
  (response) => {
    console.log("🟢 [FastAPI Client] Response success:", {
  ...
  async (error: AxiosError) => {
    console.error("🔴 [FastAPI Client] Response error:", {
