...
export default function Station5Schedule({
  assessmentData,
  ...
  const wasEditingRef = useRef<boolean>(
    !!assessmentId && !createdAssessmentUrl,
  );
  const [accessMode, setAccessMode] = useState<"private" | "public">(
    assessmentData.accessMode || "private",
  );
  const [examMode, setExamMode] = useState<"strict" | "flexible">(
    assessmentData.examMode || "strict",
  );
  const scheduleStartTime =
    (assessmentData as any)?.schedule?.startTime || assessmentData.startTime;
  const scheduleEndTime =
    (assessmentData as any)?.schedule?.endTime || assessmentData.endTime;
  const scheduleDuration =
    (assessmentData as any)?.schedule?.duration || assessmentData.duration;
  ...
  const [passPercentage, setPassPercentage] = useState(
    assessmentData.passPercentage?.toString() || "50",
  );

  const enablePerSectionTimers = useMemo(
  ...
  const [candidateCountWhenSent, setCandidateCountWhenSent] =
    useState<number>(0);

  const [requirePhone, setRequirePhone] = useState(
    (assessmentData as any)?.schedule?.candidateRequirements?.requirePhone ??
      false,
  );
  const [requireLinkedIn, setRequireLinkedIn] = useState(
    (assessmentData as any)?.schedule?.candidateRequirements?.requireLinkedIn ??
      false,
  );
  const [requireGithub, setRequireGithub] = useState(
    (assessmentData as any)?.schedule?.candidateRequirements?.requireGithub ??
      false,
  );
  ...
  useEffect(() => {
    ...
    if (proctoringSettings) {
      ...
      setLiveProctoringEnabled(
        proctoringSettings.liveProctoringEnabled ?? false,
      );
    }
  ...
  useEffect(() => {
    if (enablePerSectionTimers && sectionTimers) {
      const totalMinutes =
        (sectionTimers.MCQ || 0) +
        (sectionTimers.Subjective || 0) +
        (sectionTimers.Coding || 0);
      if (totalMinutes > 0) setDuration(totalMinutes.toString());
    }
 ...
 const handleSendInvitations = async (template: any) => {
  ...
  try {
    const result = await customMCQApi.sendInvitations(
      assessmentId,
      candidates,
      createdAssessmentUrl,
      template,
    ...
    console.log("Invitation result:", JSON.stringify(result, null, 2));  // ← ADD THIS
    ...
    <div className="max-w-5xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700 pb-10">
      ...
      <div className="space-y-2 px-1">
        ...
        <p className="text-gray-600 text-lg font-medium leading-relaxed max-w-2xl">
          Configure access mode, exam timing, and pass percentage for your
          assessment.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
              ...
              {[
                {
                  ...
                  label: "Private",
                  icon: <Lock size={18} />,
                  desc: "Only candidates you add can take the test",
                },
                {
                  id: "public",
                  label: "Public",
                  icon: <Globe size={18} />,
                  desc: "Anyone with the link can take the test",
                },
              ].map((mode) => (
                <label
                  key={mode.id}
                  className={`relative flex flex-col p-5 rounded-2xl border-2 transition-all cursor-pointer ${accessMode === mode.id ? "bg-[#E8FAF0] border-[#0A5F38] ring-4 ring-[#0A5F38]/5" : "bg-white border-gray-100 hover:border-[#A8E8BC]"}`}
                >
                  <div className="flex items-center justify-between mb-2">
                    <div
                      className={`p-2 rounded-lg ${accessMode === mode.id ? "bg-[#0A5F38] text-white" : "bg-gray-100 text-gray-400"}`}
                    >
                      {mode.icon}
                    </div>
                    <input
                      ...
                      className="w-5 h-5 text-[#0A5F38] border-gray-300 focus:ring-[#0A5F38]"
                    />
                  </div>
            ...
            <div className="space-y-6">
                ...
                {[
                  {
                    id: "strict",
                    label: "Strict Window",
                    desc: "Starts at fixed time, ends after duration.",
                  },
                  {
                    id: "flexible",
                    label: "Flexible Window",
                    desc: "Start anytime within window, full duration given.",
                  },
                ].map((mode) => (
                  <button
                    key={mode.id}
                    ...
                    className={`text-left p-4 rounded-2xl border-2 transition-all ${examMode === mode.id ? "bg-[#E8FAF0] border-[#0A5F38]" : "bg-gray-50 border-transparent hover:bg-gray-100"}`}
                  >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 pt-4 border-t border-gray-50">
                <div className="space-y-2">
                  ...
                  <input
                    ...
                    className="w-full px-4 py-3 bg-gray-50 border-2 border-transparent rounded-xl focus:bg-white focus:border-[#0A5F38] outline-none font-bold text-gray-700 transition-all shadow-inner"
                  />
                </div>
                ...
                {examMode === "flexible" && (
                  <div className="space-y-2">
                    <label className="text-xs font-black text-gray-400 uppercase tracking-widest px-1">
                      Schedule End Time <span className="text-red-500">*</span>
                    </label>
                    <input
                      type="datetime-local"
                      value={endTime}
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    <input
                      ...
                      className={`w-full px-4 py-3 bg-gray-50 border-2 border-transparent rounded-xl focus:bg-white focus:border-[#0A5F38] outline-none font-bold text-gray-700 transition-all shadow-inner ${enablePerSectionTimers ? "opacity-60 cursor-not-allowed" : ""}`}
                    />
                    {enablePerSectionTimers && (
                      <Zap
                        className="absolute right-3 top-1/2 -translate-y-1/2 text-[#0A5F38]"
                        size={16}
                      />
                    )}
                  </div>
                  {enablePerSectionTimers && (
                    <p className="text-[10px] text-[#0A5F38] font-bold bg-[#C9F4D4] px-2 py-1 rounded-md inline-block">
              ...
              {startTime && duration && examMode === "strict" && (
                <div className="p-4 bg-[#E8FAF0] rounded-2xl border border-[#A8E8BC]/30 flex items-center gap-3">
                  ...
                  <p className="text-xs font-bold text-[#0A5F38]">
        ...
        <div className="lg:col-span-5 space-y-8">
              ...
              {[
                {
                  ...
                  set: (v: boolean) => {
                    setAiProctoringEnabled(v);
                    if (!v) setFaceMismatchEnabled(false);
                  },
                  label: "AI Proctoring",
                ...
                <label
                  key={idx}
                  className="flex items-start gap-4 cursor-pointer p-5 bg-[#6b8e7b]/20 rounded-2xl hover:bg-[#6b8e7b]/30 transition-all border border-[#0A5F38]/10 shadow-sm"
                >
                  <input
                    type="checkbox"
                  ...
                  />
                  <div>
                    <div className="text-sm font-bold text-[#0A5F38]">
                      {item.label}
                    </div>
                    <div className="text-[10px] text-[#0A5F38]/70 mt-0.5 font-bold leading-relaxed">
                      {item.desc}
                    </div>
                  </div>
                </label>
                ...
                <div className="animate-in slide-in-from-top-2 duration-300">
                  <label className="flex items-start gap-4 cursor-pointer p-5 bg-[#6b8e7b]/20 rounded-2xl hover:bg-[#6b8e7b]/30 transition-all border border-[#0A5F38]/10 shadow-sm">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] text-[#0A5F38]/70 mt-0.5 font-bold leading-relaxed">
                        Verify identity against reference photo.
                      </p>
                    </div>
                  </label>
                </div>
          ...
          <section className="bg-white rounded-[2rem] p-8 border-2 border-[#A8E8BC]/20 shadow-sm space-y-8">
            <div>
              ...
              <div className="flex items-center gap-4 bg-gray-50 p-4 rounded-2xl">
                <input
                  ...
                  className="flex-1 accent-[#0A5F38]"
                />
                <span className="text-2xl font-black text-[#0A5F38] w-16 text-right">
                  {passPercentage}%
                </span>
              </div>
            </div>
            ...
            <div>
              <h3 className="text-lg font-bold text-gray-900 mb-4 flex items-center gap-2">
                <UserCheck size={18} className="text-[#0A5F38]" /> Candidate
                Requirements
              </h3>
              <div className="grid grid-cols-1 gap-3">
                {[
                  ...
                  },
                  {
                    ...
                    set: setRequireLinkedIn,
                    label: "LinkedIn URL",
                    icon: <Linkedin size={14} />,
                  },
                  {
                    checked: requireGithub,
                    set: setRequireGithub,
                    label: "GitHub URL",
                    icon: <Github size={14} />,
                  },
                ].map((req, i) => (
                  <label
                    key={i}
                    className={`flex items-center justify-between p-3.5 rounded-xl border-2 transition-all cursor-pointer ${req.checked ? "bg-[#E8FAF0] border-[#0A5F38]" : "bg-gray-50 border-transparent hover:bg-gray-100"}`}
                  >
                    ...
                    <input
                      ...
                      className="w-4 h-4 text-[#0A5F38] border-gray-300 rounded focus:ring-0"
                    />
                  </label>
                ))}
              </div>
      ...
      <div className="pt-6">
        <div
          className={`p-8 rounded-[2.5rem] border-2 transition-all duration-500 ${createdAssessmentUrl ? "bg-[#E8FAF0] border-[#0A5F38]" : "bg-[#C9F4D4] border-[#0A5F38]/10 shadow-2xl shadow-[#C9F4D4]/40"}`}
        >
          {!createdAssessmentUrl ? (
            <div className="flex flex-col md:flex-row items-center justify-between gap-6">
              ...
              <button
                ...
                disabled={loading}
                className="w-full md:w-auto px-12 py-5 bg-[#0A5F38] text-white rounded-3xl font-black text-lg uppercase tracking-wider shadow-xl hover:scale-[1.02] active:scale-95 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {loading ? (
            ...
            <div className="space-y-8 animate-in zoom-in-95">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-[#0A5F38]/70 font-bold">
                    Your assessment is live and ready for candidates.
                  </p>
                </div>
              </div>

              <div className="space-y-3">
                <label className="text-xs font-black text-[#0A5F38] uppercase tracking-widest px-1 opacity-60">
                  Assessment Shareable Link
                </label>
                <div className="flex gap-3 items-center bg-white p-2 rounded-[1.5rem] border-2 border-[#0A5F38]/20 shadow-inner">
                  <input
                    ...
                    readOnly
                    className="flex-1 bg-transparent px-4 py-2 font-bold text-gray-600 outline-none text-sm overflow-hidden text-ellipsis"
                  />
                  <button
                    onClick={() => {
                    ...
                    className="p-3 bg-[#C9F4D4] text-[#0A5F38] rounded-xl hover:bg-[#0A5F38] hover:text-white transition-all shadow-sm"
                  >
              ...
              <div className="flex flex-col sm:flex-row gap-4">
                {assessmentData.candidates &&
                  assessmentData.candidates.length > 0 && (
                    <button
                      type="button"
                      onClick={() => setIsEmailModalOpen(true)}
                      disabled={
                        sendingEmails || (invitationsSent && !hasNewCandidates)
                      }
                      className={`
  ...
  `}
                ...
                {router && (
                  <button
                    onClick={() => router.push("/assessments?refresh=true")}
                    className="px-10 py-5 bg-white border-2 border-gray-100 rounded-[1.5rem] font-black uppercase text-sm tracking-widest text-gray-500 hover:bg-gray-50 flex items-center justify-center gap-2 transition-all"
                  >
                    <LayoutDashboard size={20} /> Finish
                  </button>
                )}
              </div>
            </div>
