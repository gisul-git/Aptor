...
export default function DevOpsAssessmentsPage() {
  ...
  const devopsAssessments = useMemo(
    () => (Array.isArray(assessments) ? assessments.filter((a) => a.type === "devops") : []),
    [assessments]
  );
  const [fallbackDevopsAssessments, setFallbackDevopsAssessments] = useState<Assessment[]>([]);
  const mergedDevopsAssessments = useMemo(() => {
  ...
  }, [devopsAssessments, fallbackDevopsAssessments]);
  ...
  }, [updateSession, refetchDevOps]);

  useEffect(() => {
    if (router.query.refresh) {
      refetchDevOps();
      router.replace("/devops/assessments", undefined, { shallow: true });
    }
  }, [router.query.refresh, refetchDevOps, router]);

  useEffect(() => {
    let active = true;
    const loadFallbackDevOps = async () => {
      try {
  ...
  const confirmDeleteAssessment = async () => {
    ...
    try {
      ...
      setUiError(null);
      await deleteDevOpsTestMutation.mutateAsync(deleteModal.assessmentId);
      setDeleteModal({
        isOpen: false,
        assessmentId: null,
        assessmentTitle: "",
  ...
  const handlePauseAssessment = async (assessmentId: string, e: React.MouseEvent) => {
    e.stopPropagation();
    setOpenMenuId(null);
    const assessment = mergedDevopsAssessments.find((a) => a.id === assessmentId);
    const testName = assessment?.title || "DevOps Assessment";
      ...
      <div className="assessments-container relative z-10" style={{ paddingTop: "6rem" }}>
        {error || uiError ? (
          <div className="card">
            ...
            <button type="button" className="btn-primary" onClick={() => { refetchDevOps(); setUiError(null); }} style={{ marginTop: "1rem" }}>
              Retry
            </button>
          </div>
        ) : (
          <AssessmentsList
            assessments={mergedDevopsAssessments}
