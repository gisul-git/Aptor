...
import { cloudCandidateApi } from "@/lib/cloud/api";
...
export default function CloudEntryPage() {
  ...
  const handleVerify = async (event: FormEvent) => {
    ...
    try {
      ...
      const result = await cloudCandidateApi.verifyCandidate(testId, token, cleanEmail, cleanName);
      if (!result?.verified) {
      ...
      setGateContext({
        ...
        candidateUserId,
        entryUrl: `/cloud/tests/${testId}/entry?token=${encodeURIComponent(token)}`,
        finalTakeUrl: `/cloud/tests/${testId}/take?token=${encodeURIComponent(token)}`,
      });
  ...
  return (
    ...
    >
      ...
      >
        <h1 style={{ margin: 0, marginBottom: "0.5rem", color: "#114b2f" }}>Cloud Test Access</h1>
