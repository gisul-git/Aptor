...
const CLOUD_TERMINAL_WS_URL = "ws://103.173.99.254:4040/terminal";
...
export default function CloudTakePage() {
  ...
  useEffect(() => {
    ...
    const entryUrl = `/cloud/tests/${testId}/entry?token=${encodeURIComponent(token)}`;
    const instructionsUrl = `/cloud/tests/${testId}/instructions?token=${encodeURIComponent(token)}`;
  ...
  useEffect(() => {
    ...
    const setupTerminal = async () => {
      ...
      const terminal = new Terminal({
        ...
        disableStdin: false,
        fontFamily: "var(--font-cloud-mono), 'JetBrains Mono', monospace",
        fontSize: 14,
        lineHeight: 1.4,
        convertEol: false,
        theme: {
  ...
  const buildTerminalWsUrls = useCallback((): string[] => {
    const fallback =
      typeof window !== "undefined"
        ...
        : "";
    return [CLOUD_TERMINAL_WS_URL, fallback].filter(Boolean);
  }, []);
  ...
  useEffect(() => {
    ...
    terminalInputDisposableRef.current = terminal.onData((data: string) => {
      ...
      // Keep typing feedback visible even before websocket is connected.
      try {
        if (data === "\u007f") {
          terminal.write("\b \b");
        } else if (data === "\r") {
        ...
        // Ignore local echo failures; buffered send path continues.
      }
  ...
  const submitExecutionSession = useCallback(async (sessionId: string) => {
    ...
    try {
      await fetch("/api/cloud/submit-session", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const resetExecutionSession = useCallback(async (sessionId: string): Promise<string> => {
    ...
    if (!safeSessionId) return "";
    try {
      const response = await fetch("/api/cloud/reset-session", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ session_id: safeSessionId }),
      });
