...
export default function CloudInstructionsPage() {
  ...
  const { data: testData } = useCloudTest(testId);
  ...
  const handleContinue = async () => {
    ...
    await router.replace(`/cloud/tests/${testId}/take?token=${encodeURIComponent(token)}`);
  };
  ...
  return (
    ...
    >
      ...
      >
        <h1 style={{ marginTop: 0, color: "#0f172a" }}>Cloud Assessment Instructions</h1>
        {name && email ? (
