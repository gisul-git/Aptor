...
function normalizePayload(q: QuestionLike, idx: number) {
  return {
    ...
    description: String(q.description || "DevOps task"),
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const devopsBase =
    ...
    process.env.DEVOPS_EXECUTION_API_URL ||
    "http://127.0.0.1:8010";

  try {
    const results = await Promise.all(
      questions.map(async (q, idx) => {
        const response = await fetch(`${devopsBase.replace(/\/+$/, "")}/api/v1/devops/questions/`, {
          method: "POST",
        ...
        if (!response.ok) {
          return {
            ...
            question: { ...q, id: String(q.id || `generated-devops-${idx + 1}`), persisted: false },
          };
        }
        ...
        return {
          ...
          question: {
            ...q,
            id: String(saved?.id || q.id || `generated-devops-${idx + 1}`),
            persisted: true,
            is_published: false,
          },
        };
