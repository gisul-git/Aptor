// Data Engineering Test Management - List Page
// Reused from DSA competency with Data Engineering specific adaptations
...
export default function DataEngineeringTestsListPage() {
  const router = useRouter()
  const [tests, setTests] = useState<Test[]>([])
  const [loading, setLoading] = useState(true)
  const queryClient = useQueryClient()
  
  // Modal and candidate state
  ...
  const handlePublish = async (testId: string, currentStatus: boolean) => {
    try {
      ...
      await apiClient.patch(`/api/v1/data-engineering/tests/${testId}/publish?is_published=${newStatus}`)
      await fetchTests()
    } catch (error: any) {
  ...
  const handleAddCandidate = async (testId: string) => {
    ...
    try {
      const response = await apiClient.post(`/api/v1/data-engineering/tests/${testId}/candidates`, {
  ...
  const handleBulkUpload = async (testId: string, file: File) => {
    ...
    try {
      const response = await apiClient.post(`/api/v1/data-engineering/tests/${testId}/candidates/bulk`, formData, {
        headers: {
      ...
      alert(
        ...
        `✅ Added: ${responseData?.added?.length || 0}\n` +
        `⚠️ Skipped: ${responseData?.skipped?.length || 0}`
      )
        ...
        <div style={{ marginBottom: "2.5rem", display: "flex", justifyContent: "space-between", alignItems: "flex-end" }}>
          <div>
            ...
            <p style={{ color: "#6B7280", fontSize: "1rem", margin: 0 }}>
              Manage your Data Engineering assessments, invite candidates, and review configurations.
            </p>
          </div>
          <button
        ...
        {tests.length === 0 ? (
          <div style={{ backgroundColor: "#ffffff", padding: "4rem 2rem", borderRadius: "1rem", border: "1px dashed #D1D5DB", textAlign: "center" }}>
            ...
            <p style={{ margin: 0, color: "#6B7280", fontSize: "0.95rem" }}>
              You haven't created any tests yet. Head to the dashboard to create one.
            </p>
          </div>
        ) : (
                ...
                <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: "2rem", flexWrap: "wrap" }}>
                    ...
                    {test.is_published && test.test_token && !test.isExpired && (
                      <div style={{ padding: "1rem", backgroundColor: "#F0F9F4", border: "1px solid #A8E8BC", borderRadius: "0.5rem", marginTop: "1rem" }}>
                        ...
                        <div style={{ display: "flex", gap: "0.5rem" }}>
                          <input
                            type="text"
                            value={`${typeof window !== 'undefined' ? window.location.origin : ''}/data-engineering/test/${test.id}?token=${test.test_token}`}
                            readOnly
                          ...
                          <button
                            type="button"
                            onClick={async () => {
                              const link = `${window.location.origin}/data-engineering/test/${test.id}?token=${test.test_token}`
                              try {
                  ...
                  <div style={{ width: "260px", display: "flex", flexDirection: "column", gap: "0.5rem" }}>
                    ...
                    <button
                      ...
                      title={test.isExpired ? "Assessment has ended" : !test.is_published ? "Publish test to add candidates" : "Add and manage candidates"}
                      style={{ 
                    ...
                    >
                      <Users size={18} /> Add Candidates
                    </button>

                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0.5rem" }}>
                      <button
                        onClick={() => router.push(`/data-engineering/tests/${test.id}/candidates`)}
                        style={{ 
                      ...
                      <button
                        onClick={() => router.push(`/data-engineering/tests/${test.id}/edit`)}
                        style={{ 
                          display: "flex", alignItems: "center", justifyContent: "center", gap: "0.375rem",
                          padding: "0.625rem", fontSize: "0.875rem", fontWeight: 600, color: "#374151", 
                          backgroundColor: "#ffffff", border: "1px solid #D1D5DB", borderRadius: "0.5rem", 
